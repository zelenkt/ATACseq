...
Example usage:
    sbatch --chdir=/share/lab_avram/HPC_Cluster/user/tomas/06_ATACseq_murine_tumors_Leo_Tomas_02-02-2023/results/CD8_cells/tumor --export=s=/share/lab_avram/HPC_Cluster/user/tomas/06_ATACseq_murine_tumors_Leo_Tomas_02-02-2023/tools/samples_C8T.csv ~/bin/pepatac_DESeq2_for_genrich.sh"
...
##### SET VARIABLES #####
target_dir="${d:-${PWD}/DESeq2_genrich}"
if [[ "$target_dir" == "$PWD/DESeq2_genrich" && ! -d "$target_dir" ]]; then # IF TARGET DIR DOES NOT EXIST
    mkdir -p "$target_dir"
...
samples="${s:-/share/lab_avram/HPC_Cluster/user/tomas/06_ATACseq_murine_tumors_Leo_Tomas_02-02-2023/tools/samples_C8T.csv}"  # samples="${r:-${PWD}/DESeq2/test-samples.csv}"
...
## Extract the read_count values from individual ref_coverage files - coverage of the WT+KO consensus peaks generated by project level pepatac
#samples="/share/lab_avram/HPC_Cluster/user/tomas/06_ATACseq_murine_tumors_Leo_Tomas_02-02-2023/tools/samples_C4P.csv"; 
if [[ ! -f ${target_dir}/raw_read_counts_pepatac_consensus_peaks.txt ]]; then
    printf "Extracting raw read counts for consensus peaks from individual samples...\n" "$base"

    # note that for MACS2 output I should print awk '{print $11} but for genrich it is awk '{print $10}
    sampleCount=$(awk -F',' 'NR>1{print $1}' $samples | wc -l); awk -F',' 'NR>1{print $1}' $samples | sort > tmp_4245277sampleNames.txt; for ((sampleOrder=1; sampleOrder<=$sampleCount; sampleOrder++)); do sampleName=$(sed -n ${sampleOrder}p tmp_4245277sampleNames.txt); awk '{print $10}' ${PWD}/results_pipeline/${sampleName}/peak_calling_${deseq2_genome}/genrich_ref_coverage/${sampleName}_ref_peaks_coverage.bed > tmp_3541643543534154_${sampleName}; echo -e '' > tmp_9878798798798_consensus_${sampleOrder}; awk -v OFS="_" '{print $1,$2,$3}' ${PWD}/results_pipeline/${sampleName}/peak_calling_${deseq2_genome}/genrich_ref_coverage/${sampleName}_ref_peaks_coverage.bed >> tmp_9878798798798_consensus_${sampleOrder}; done; 
    awk '
    { 
        for (i=1; i<=NF; i++)  {
            a[NR,i] = $i
    ...
    }' tmp_4245277sampleNames.txt | sed -e 's/ /\t/g' > tmp_13213213213132sampleNames.txt; paste tmp_3541643543534154* > tmp999898018194245242; cat tmp_13213213213132sampleNames.txt tmp999898018194245242 > tmp8184414654654read_counts.txt; rm tmp_13213213213132sampleNames.txt; rm tmp_3541643543534154*; rm tmp_4245277sampleNames.txt; rm tmp999898018194245242; paste tmp_9878798798798_consensus_1 tmp8184414654654read_counts.txt | awk '!($1 ~ /^chrUn/ || $1 ~ /random/ || $1~/^chrM/ || $1~/^chrY/) {print $0}' > raw_read_counts_pepatac_consensus_peaks.txt; rm tmp_9878798798798_consensus*; rm tmp8184414654654read_counts.txt; mv raw_read_counts_pepatac_consensus_peaks.txt "$target_dir"/
...
readcounts="${r:-${target_dir}/raw_read_counts_pepatac_consensus_peaks.txt}"
