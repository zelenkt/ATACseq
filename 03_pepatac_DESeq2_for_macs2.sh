...
Example usage:
    sbatch --chdir=/share/lab_avram/HPC_Cluster/user/tomas/06_ATACseq_murine_tumors_Leo_Tomas_02-02-2023/results/CD8_cells/tumor --export=s=/share/lab_avram/HPC_Cluster/user/tomas/06_ATACseq_murine_tumors_Leo_Tomas_02-02-2023/tools/samples_C8T.csv ~/bin/pepatac_DESeq2_for_macs2.sh"
...
##### SET VARIABLES #####
target_dir="${d:-${PWD}/DESeq2_macs2}"
if [[ "$target_dir" == "$PWD/DESeq2_macs2" && ! -d "$target_dir" ]]; then # IF TARGET DIR DOES NOT EXIST
    mkdir -p "$target_dir"
...
if [[ ! -f ${target_dir}/raw_read_counts_pepatac_consensus_peaks.txt ]]; then
    ...
    sampleCount=$(awk -F',' 'NR>1{print $1}' $samples | wc -l); awk -F',' 'NR>1{print $1}' $samples | sort > tmp_4245277sampleNames.txt; for ((sampleOrder=1; sampleOrder<=$sampleCount; sampleOrder++)); do sampleName=$(sed -n ${sampleOrder}p tmp_4245277sampleNames.txt); awk '{print $11}' ${PWD}/results_pipeline/${sampleName}/peak_calling_${deseq2_genome}/macs2_fixed_ref_coverage/${sampleName}_ref_peaks_coverage.bed > tmp_3541643543534154_${sampleName}; echo -e '' > tmp_9878798798798_consensus_${sampleOrder}; awk -v OFS="_" '{print $1,$2,$3}' ${PWD}/results_pipeline/${sampleName}/peak_calling_${deseq2_genome}/macs2_fixed_ref_coverage/${sampleName}_ref_peaks_coverage.bed >> tmp_9878798798798_consensus_${sampleOrder}; done; 
    awk '
    { 
        for (i=1; i<=NF; i++)  {
            a[NR,i] = $i
